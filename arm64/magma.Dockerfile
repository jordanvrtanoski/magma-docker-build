...
RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./nettle_build.sh && \
    dpkg -i oai-nettle_2.5-1_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./asn1c_build.sh && \
    dpkg -i oai-asn1c_0~20170324+c0~r224dc1f9-0_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./folly_build.sh && \
    dpkg -i libfolly-dev_2018.02.26.00-6_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./magma-libtacopie_build.sh && \
    dpkg -i magma-libtacopie_3.2.0.1-1_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./magma-cpp-redis_build.sh && \
    dpkg -i magma-cpp-redis_4.3.1.1-2_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./liblfds_build.sh && \
    dpkg -i liblfds710_7.1.0-0_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./libfluid_build.sh && \
    dpkg -i magma-libfluid_0.1.0.5-1_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./grpc_build.sh && \
    dpkg -i grpc-dev_1.15.0-3_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./gnutls_build.sh && \
    dpkg -i oai-gnutls_3.1.23-1_arm64.deb

RUN cd /magma/third_party/build/bin/ && \
    ARCH=arm64 ./prometheus-cpp_build.sh && \
...
RUN cd /magma/lte/gateway/release/ && PYTHON_BUILD=/tmp/python-build/build PIP_CACHE_HOME=/tmp/python-build/cache MAGMA_ROOT=/magma ./build-magma.sh -a arm64 -c /rootCA.pem -h `git rev-parse --short HEAD`
