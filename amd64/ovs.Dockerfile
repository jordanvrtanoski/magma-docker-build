...
RUN apt update && \
    apt install -y git python-minimal aptitude sudo linux-image-4.9.0-9-amd64 linux-headers-4.9.0-9-amd64
...
RUN cd magma && \
    ...
    MAGMA_ROOT=/magma KVERS="4.9.0-9-amd64" ARCH="amd64" bash build-ovs.sh
